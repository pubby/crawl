...
class UIMenuScroller : public UIScroller
{
public:
    UIMenuScroller() : UIScroller() {};
    virtual ~UIMenuScroller() {};
    virtual void _allocate_region() override {
        m_child->set_allocation_needed();
        UIScroller::_allocate_region();
    };
};

class UIShowHide : public UIBin
{
public:
    UIShowHide() : UIBin() {};
    virtual ~UIShowHide() {};
...
#ifdef USE_TILE_LOCAL
    int more_height = m_menu->m_ui.more_bin->_get_preferred_size(UI::VERT,
            m_region[2]).nat;
    ...
    alive(false), last_selected(-1), m_kmc(kmc), m_filter(nullptr)
{
    ...
    m_ui.title = make_shared<UIText>();
    m_ui.more = make_shared<UIText>();
    m_ui.more_bin = make_shared<UIShowHide>();
    m_ui.vbox = make_shared<UIBox>(UI::VERT);
    m_ui.vbox->align_items = UI::STRETCH;
...
{
    m_ui.more->set_text(fs);
    m_ui.more_bin->set_visible(!fs.ops.empty());
}
...
{
    bool done = false;
    m_ui.popup = make_shared<UIPopup>(m_ui.vbox);

    m_ui.menu->on(UI::slots.event, [this, &done](wm_event ev) {
    ...
    update_menu();
    ui_push_layout(m_ui.popup, m_kmc);
